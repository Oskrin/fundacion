<?php 
	...
	if (isset($_POST['Modificar']) == "Modificar") {
		...
		if($dirFoto == '') {
			$resp = $class->consulta("UPDATE clientes SET	id_tipo_documento = '$_POST[select_documento]',
	...
	if (isset($_POST['comparar_identificacion'])) {
		...
		$resultado = $class->consulta("SELECT * FROM clientes C WHERE C.id_tipo_documento = '$_POST[tipo_documento]' AND C.identificacion = '$_POST[identificacion]'");
		while ($row = $class->fetch_array($resultado)) {
			$cont++;
		}
